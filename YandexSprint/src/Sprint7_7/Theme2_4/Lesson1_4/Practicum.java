package Sprint7_7.Theme2_4.Lesson1_4;
...
class Practicum {
    ...
    public static void main(String[] args) {
        ...
        if (currentTime.plusSeconds(ONE_DAY_SECONDS).isBefore(timeToWakeUp)) {
            System.out.printf("Дней до экзамена: %.0f. До экзамена ещё слишком далеко, волноваться не о чем!%n", calculateDuration(examStart, currentTime, TimeUnits.DAYS));
            return;
        }
        ...
        System.out.printf(
                ...
                calculateDuration(timeToWakeUp, timeToGoToSleep, TimeUnits.HOURS)
        );
    ...
    private static double calculateDuration(Instant firstMoment, Instant secondMoment, TimeUnits unit) {
        switch (unit) {
            ...
            case SECONDS: {
                return  (secondMoment.toEpochMilli() - firstMoment.toEpochMilli()) / ONE_SECOND_MILLIS;
            }
            case MINUTES: {
                return  (secondMoment.toEpochMilli() - firstMoment.toEpochMilli()) / ONE_MINUTE_MILLIS;
            }
            case HOURS: {
                return  (secondMoment.toEpochMilli() - firstMoment.toEpochMilli()) / ONE_HOUR_MILLIS;
            }
            case DAYS: {
                return  (secondMoment.toEpochMilli() - firstMoment.toEpochMilli()) / ONE_DAY_MILLIS;
            }
            default:
                return 0;
        }
