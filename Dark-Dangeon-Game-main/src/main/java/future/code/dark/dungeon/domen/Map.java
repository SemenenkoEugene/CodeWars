...
public class Map {
    ...
    private Boolean validate(List<String> lines) {
        char[] leftColumn = lines.stream()
                .map(item -> item.toCharArray()[0])
                ...
                .toCharArray();
        char[] rightColumn = lines.stream()
                .map(item -> item.toCharArray()[item.length()-1])
                .collect(Collector.of(
                StringBuilder::new,
                StringBuilder::append,
                StringBuilder::append,
        ...
        return containsExit(lines) && containsPlayer(lines) && lines
                .stream()
                .allMatch(line -> line.length() == lines.get(0).length());
    }
