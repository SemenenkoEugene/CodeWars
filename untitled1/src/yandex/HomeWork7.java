...
public class HomeWork7 {
    /*
        «јƒјЌ»≈
        Ќапишите реализации дл¤ методов использу¤ StreamAPI
        ¬ этих задани¤ «јѕ–≈ўј≈“—я использование for и if
        “ело каждого метода должно начинатьс¤ с ключевого слова return
            (т.е. задани¤ должны быть выполнены за одно выражение)
     */
    ...
    public static List<String> methodOne(List<String> names) {
        // Твой код здесь
        return names.stream()
                .filter(str -> str.contains("King"))
                .collect(Collectors.toList());
    }

    /**
     * Метод должен вернуть список уникальных фамилий (2 позиции в строке)
     */
    public static List<String> methodTwo(List<String> names) {
        // Твой код здесь
        return names.stream()
                .map(s -> s.replaceFirst("\\w+\\s", ""))
                .distinct()
                .collect(Collectors.toList());
    }
    ...
    public static List<String> methodThree(List<String> names) {
        // Твой код здесь
        return names.stream()
                .map(s -> s.replaceFirst(s.substring(s.indexOf(" ")), ""))
    ...
    /**
     * Метод должен вернуть количество имен длиннее чем 6 символов
     */
    public static Integer methodFive(List<String> names) {
        // Твой код здесь
        return Math.toIntExact(names.stream()
                .map(s -> s.replaceFirst(s.substring(s.indexOf(" ")), ""))
                .filter(s -> s.length() > 6)
                .distinct()
                .count());
    ...
    /**
     * Метод должен вернуть количество имен и фамилий (по отдельности) длиннее чем 5 символов
     */
    ...
    /**
     * Метод должен вернуть имена людей, чьи фамилии начинается с K или S в формате имя + первая буква фамилии
    ...
    public static List<String> methodSeven(List<String> names) {
        // Твой код здесь
        return names.stream()
                .filter(s -> s.matches("[a-zA-Z].+\\s[SK].+"))
                .map(s -> s.replaceAll("\\s\\w+", " " + s.charAt(s.indexOf(" ") + 1) + "."))
                .sorted()
                .distinct()
                .collect(Collectors.toList());
    }
